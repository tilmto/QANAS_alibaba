...
from autoai2c.nas_ene_feedback import ene_lat_qury
...
class ConvBlock(nn.Module):
    ...
    @staticmethod
    def _energy(layer_id, h, w, C_in, C_out, expansion, kernel_size=3, stride=1, padding=None, dilation=1, groups=1, bias=False):
        ...
        for i, conv_info in enumerate(conv_info_list):
            energy_part, latency_part = ene_lat_qury(conv_info, 3 * layer_id - 2 + i)
            energy += energy_part
            latency += latency_part
    ...
    def forward_energy(self, size):
        ...
        if name in energy_lookup_table:
            ...
            energy = ConvBlock._energy(self.layer_id, h_in, w_in, c_in, c_out, self.expansion, self.kernel_size, self.stride, self.padding, self.dilation, self.groups, self.bias)
    ...
    def forward_energy(self, size):
        ...
        if name in energy_lookup_table:
            ...
            if self.stride == 2 or self.C_in != self.C_out:
                energy = Skip._energy(self.layer_id, h_out, w_out, c_in, c_out, self.stride)
            else:
                energy = 0
            energy_lookup_table[name] = energy
            np.save(energy_file_name, energy_lookup_table)
...
class ConvNorm(nn.Module):
    ...
    @staticmethod
    def _energy(h, w, C_in, C_out, kernel_size=3, stride=1, padding=None, dilation=1, groups=1, bias=False):
        # layer = Conv(C_in, C_out, kernel_size, stride, padding, dilation, groups, bias, slimmable=False)
        # energy = compute_energy(layer, (1, C_in, h, w))
        ...
        conv_info_list = [
                     [[1, {'ch_out':[C_out,0],'ch_in':[C_in,0],'batch':[1,0],'col_out':[h,0],
        ...
        for i, conv_info in enumerate(conv_info_list):
            energy_part, latency_part = ene_lat_qury(conv_info, 0)
            energy += energy_part
            latency += latency_part
    ...
    def forward_energy(self, size):
        ...
        if name in energy_lookup_table:
            ...
            print("not found in energy_lookup_table:", name)
            energy = ConvNorm._energy(h_in, w_in, c_in, c_out, self.kernel_size, self.stride, self.padding, self.dilation, self.groups, self.bias)
            energy_lookup_table[name] = energy
            np.save(energy_file_name, energy_lookup_table)
