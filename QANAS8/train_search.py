        ...
        if update_arch:
            ...
            if (step+1) % 10 == 0:
                ...
                if config.efficiency_metric == 'flops':
                    ...
                    logger.add_scalar('arch/energy_supernet', architect.energy_supernet, epoch*len(pbar)+step)
        ...
        if criteria is not None:
            ...
            if bit_schedule == 'sandwich':
                ...
                if distill_weight > 0:
                    ...
                    logit = model(input, num_bits_max, temp=temp)
...
def infer(epoch, model, test_loader, logger, num_bits_list, finalize=False, temp=1):
    ...
    if finalize:
        ...
        if config.efficiency_metric == 'flops':
            ...
            energy = model_infer.forward_energy((3, 32, 32))
            del model_infer
